...
double calc_prob(string a, string b) {
    double ans = msc(a, b)/(b.size() + 0.0);
    ans += 1 - cnt_partes()/(b.size() + 0.0);
...
int main(int n, char *args[]) {
    ...
    while (getline (f2, linea)) {
        ...
        tipos[linea.substr(0, pcoma)].push_back(linea.substr(pcoma + 1));
    }
    ...
    while (getline(f1, linea)) {
        ...
        double mx = -1;
        string grupo;
        for (typeof tipos.begin() it = tipos.begin(); it != tipos.end(); it++) {
            double nmx = calc_prob(pls[1], it->first);
            if (nmx > mx) {
                grupo = it->first;
                mx = nmx;
            }
        ...
        mx = -1.0;
        string op;
        vector <string> list = tipos[grupo];
        for (typeof list.begin() it = list.begin(); it != list.end(); it++) {
            double nmx = calc_prob(pls[1], *it);
            if (nmx >= mx) {
                mx = nmx;
                op = *it;
            }
