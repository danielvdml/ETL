...
exit=True
cont=0
data.write("id|origen|titulo|link|precio|moneda|monedaSimbolo|condicion|imagen|lugar")
while exit:
	i=i+1
	if i%20==0:
		time.sleep(60)
	...
	if len(items)>0:
		for item in items:		
			...
			s=str(cont)+"|"+"olx"+"|"+titulo+"|"+link+"|"+precio+"|"+moneda+"|"+monedaSimbolo+"|"+condicion+"|"+imagen+"|"+lugar+"\n"
			data.write(s)
			print(str(i)+" "+s)
	else:
		exit=False
